...
pub(crate) fn save_replay<'e, E: ExecutionState<'e>>(
    ...
    for str_ref in &str_refs {
        let entry_result = entry_of_until(binary, &funcs, str_ref.use_address, |entry| {
            ...
            let mut replay_analyzer = SaveReplayFunctionAnalyzer::<E> {
                result: None,
                arg_cache: &analysis.arg_cache,
                state: SaveReplayState::FindFilenameCall,
                filepath_operand: None,
            };
            let mut target_analysis = FuncAnalysis::new(binary, ctx, target_func);
            target_analysis.analyze(&mut replay_analyzer);
            
            match replay_analyzer.result {
                Some(result) => EntryOf::Ok(result),
                None => EntryOf::Retry,
            }
        }).into_option();
